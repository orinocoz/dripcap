import {IPv6Address, Enum, Flags} from 'dripper/type'

export default class IPv6Decoder {
  constructor() {
    this.lowerLayers = ['::Ethernet::<IPv6>']
  }
  ...
  analyze(packet) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        assertLength(4)
        let flowLevel = payload.readUInt16BE(2, true) |
        ...
        assertLength(6)
        let payloadLength = payload.readUInt16BE(4, true)
        layer.fields.push({
          name: 'Payload Length',
          attr: 'payloadLength',
        ...
        assertLength(7)
        let nextHeader = payload.readUInt8(6, true)
        let nextHeaderRange = slice.slice(6, 7)
        ...
        assertLength(24)
        let source = new IPv6Address(payload.slice(8, 24))
        layer.fields.push({
        ...
        assertLength(40)
        let destination = new IPv6Address(payload.slice(24, 40))
        layer.fields.push({
        ...
        while (ext) {
          let optlen = 0
          switch (nextHeader) {
            case 0: case 60:  // Hop-by-Hop Options, Destination Options
              let extLen = (payload.readUInt8(offset + 1, true) + 1) * 8
        ...
        let protocol = new Enum(protocolTable, nextHeader)
